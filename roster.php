    ...
    <?php
      if (!isset($_SESSION['job'])) {
        ...
        <form method='post'>
          ...
          <label for='select_supervisor'>
          Choose Supervisor: <select name='select_supervisor' required>
        ";
          ...
          Choose Doctor:  <select name='select_doctor' required>
          ";
          ...
          <label for='caregiver_1'>
          Choose Care Giver 1: <select name='caregiver_1' required>
          ";
          ...
          <label for='caregiver_2'>
          Choose Care Giver 2: <select name='caregiver_2' required>
          ";
              $caregiver = mysqli_query($conn, $get_caregiver);
              while($row = mysqli_fetch_assoc($caregiver)) {
                  echo "<option value=\"{$row['user_id']}\">{$row['f_name']} {$row['l_name']}</option>";
          ...
          <label for='caregiver_3'>
          Choose Care Giver 3: <select name='caregiver_3' required>
          ";
              $caregiver = mysqli_query($conn, $get_caregiver);
              while($row = mysqli_fetch_assoc($caregiver)) {
                  echo "<option value=\"{$row['user_id']}\">{$row['f_name']} {$row['l_name']}</option>";
          ...
          <label for='caregiver_4'>
          Choose Care Giver 4: <select name='caregiver_4' required>
          ";
              $caregiver = mysqli_query($conn, $get_caregiver);
              while($row = mysqli_fetch_assoc($caregiver)) {
                  echo "<option value=\"{$row['user_id']}\">{$row['f_name']} {$row['l_name']}</option>";
