    ...
    <body>
        ...
        <?php
            ...
            if (!isset($_SESSION['job'])) {
                ...
                $get_roster = "SELECT roster_date FROM rosters WHERE roster_date = '$display_date';";
                $roster_details = mysqli_query($conn, $get_roster);
                $roster = mysqli_fetch_assoc($roster_details);
                ...
                if ($roster) {
                    ...
                    while($patient = mysqli_fetch_assoc($patient_details)) {
                        ...
                        if (!$patient['group_id']) {
                            $caregiver_check = 'NOT ASSIGNED';
                        } elseif ($patient['group_id']) {
                            $caregiver_details= mysqli_query($conn, $get_caregiver);
                            $caregiver = mysqli_fetch_assoc($caregiver_details);
                            $caregiver_check = $caregiver['f_name']." ".$caregiver['l_name'];
                        }
                    ...
                    echo "<p>NO ROSTER ASSIGNED YET</p>";
                }
            }
